...
server = function(input, output) {
  ...
  selector1 = eventReactive(input$run, {
    if(!is.null(file1()) && input$selector1_type == "Custom") {
      model = create_model(type = "Custom", 
                              learner_name = NULL, 
                              file_name = file1(),
                              data = scenario_data())
    } else if(input$selector1_type == "mlr/llama") {
      model = create_model(type = "mlr/llama", 
  ...
  selector2 = eventReactive(input$run, {
    if(!is.null(file2()) && input$selector2_type == "Custom") {
      model = create_model(type = input$selector2_type, 
                                         learner_name = NULL, 
                                         file_name = file2(),
                                         data = scenario_data())
  ...
  summary = reactive(summarize(input$metric, model1_gap_mcp(), model2_gap_mcp(),
                               model1_gap_par(), model2_gap_par()))
